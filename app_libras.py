            ...
            if resultado.multi_hand_landmarks:
                for hand_landmarks in resultado.multi_hand_landmarks:
                    ...
                    if len(buffer_frames) == SEQ_LENGTH:
                        tensor_seq = torch.tensor([list(buffer_frames)], dtype=torch.float32).to(DEVICE)
                        with torch.no_grad():
                            previsao = modelo(tensor_seq)
                            indice = torch.argmax(previsao).item()
                            letra_detectada = classes[indice]
                        
                        letra_atual_display.info(f"Detectado: **{letra_detectada}**")
                        
                        # Debounce de 2 segundos para registro na frase
                        if time.time() - ultimo_tempo_letra > 2:
                            if letra_detectada == 'ESPACO': frase += " "
                            elif letra_detectada == 'APAGAR': frase = frase[:-1]
                            else: frase += letra_detectada
